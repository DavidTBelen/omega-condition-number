%%% Testing Henry's conjecture sqrt(omega(A^(-2))); Nov 16
%%% Rubric note under Remark 2.3
%% currently figure 2.3 for lin regr models
...
printpdf = false;   % skip print to pdf leave as false
kind = 2; % normal shifted distribution for eigs
if kind == 1
        fprintf('using unif. distrib eigs: deigs = 10*(rand(n,1)+.1)\n');
elseif kind == 2
        fprintf('using normal distrib eigs: deigs = randn(n,1)\n');
end
...
for jj = 1:numbAs  % different matrices A_jj
   ...
   % Ainv = VA*diag(deigs.^(-1))*VA';
   % Ainvsqr = Ainv*Ainv;
   kappas(jj) = cond(A);
   % cholA = chol(A);
   invsqrdeigs = deigs.^(-2);
   % omega(jj) = ((trace(Ainvsqr)/prod(deigs.^(-2/n)))/n)^(1/2);
   %omega(jj) = (sum(invsqrdeigs)/(n*prod(invsqrdeigs.^(1/n))))^(1/2);
   omega(jj) = sqrt(sum(invsqrdeigs)/n)*(prod(deigs.^(1/n)));
   %(det_rootn(Ainvsqr)) = (1/det_rootn(A))
   %checkomega(jj) = (trace(Ainvsqr)/n)/(det_rootn(Ainvsqr));
...
%    plot(1:m,condepsm)
   ...
   if kind == 1
    title('LRM cond and \kappa for uniform distributed eigenvalues')
   elseif kind == 2
    title('LRM cond and \kappa for normally distributed eigenvalues')
   end
   ...
   if printpdf
   if kind == 1
    print(gcf,'..\Latexrevisingasofnov9COAPSubmission\lrm_kappa_temp_uniform.pdf','-dpdf','-r300','-bestfit');
   elseif kind == 2
    print(h1,'..\Latexrevisingasofnov9COAPSubmission\lrm_kappa_temp_normal.pdf','-dpdf','-r300','-bestfit');
   end
   else
	   fprintf('skipping the print fig to pdf file\n')
   end
   hold off
   ...
   legend('(cond(A_{i}),\omega(A_{i}^{-2})^{1/2})','Regression line','location','southeast')
   if kind == 1
    title('LRM cond and \omega^{-2} for uniform distributed eigenvalues')
   elseif kind == 2
    title('LRM  cond and \omega^{-2} for normally distributed eigenvalues')
   end
   xlabel('Mean of condition number estimates')
   ylabel('\omega^{-2}')
   xlim([min(condepsm),max(condepsm)])
   ...
   if printpdf
   if kind == 1
    print(gcf,'..\Latexrevisingasofnov9COAPSubmission\lrm_omega_inv_uniform.pdf','-dpdf','-r300','-bestfit');
   elseif kind == 2
    print(h2,'..\Latexrevisingasofnov9COAPSubmission\lrm_omega_inv_normal.pdf','-dpdf','-r300','-bestfit');
   end
   else
	   fprintf('skipping the print fig to pdf file\n')
   end
