...
2. performance_profile_file.m
	...
	- Plots the performance profile by running the "run_file"
	  or loading the results file.
	- It calls for "tps_performance.m" and "perf.m"
	- Saves the plots in the latex folder

3. table_file.m
	
	- Generates the tables by running the "run_file"
	  or loading the results file.
	- Choose type for obtaining the desired table
	  (time, iterations, residuals,...)
...
4. small_matrices.m
	...
	- Output is a file "small_matrices_list.mat" that can be used
	  as data for the "run_file"
...
6. folder "Auxiliary_files" contains different used functions:


	- folder "Preconditioners" contain the codes for computing the diagonal
	  and diagonal + block triangular preconditioner
